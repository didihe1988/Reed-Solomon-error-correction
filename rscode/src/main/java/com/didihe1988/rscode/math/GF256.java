...
public class GF256{
    ...
    private static int[] expOf={
            255, 0, 25, 1, 50, 2, 26, 198, 75, 199, 27, 104, 51, 238, 223,
    ...
    //byte的范围是-128~127,为使用GF(2^8),byte+128,输出时byte-128
    /*
    ...
    public GF256 inverse(){
        ...
        return new GF256(alphaTo[255-e]);
    }


    public GF256 add(GF256 other){
        return add(this.value, other.getValue());
    }

    public GF256 sub(GF256 other){
        return add(other);
    }

    public GF256 mul(GF256 other){
        return mul(this.value, other.getValue());
    }

    //和mul类似 只不过x+y --> x^n
    public GF256 pow(int n){
        ...
        int z=y%255;
        return new GF256(alphaTo[z]);
    }

    private GF256 add(int a,int b){
        //伽罗华域的加法为异或
        return new GF256(a^b);
    }
    ...
    public boolean equals(Object obj) {
        if(obj instanceof GF256){
            if(this.value==((GF256) obj).getValue()){
                return true;
            }
        }
        return false;
